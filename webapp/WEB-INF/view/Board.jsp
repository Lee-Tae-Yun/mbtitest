...
String id2 = (String)session.getAttribute("id");
	  ...
	  int endRow= currentPage * pageSize;
		//게시글 출력
	  ArrayList<BoardDto> dtos = dao.getBoardList(startRow, pageSize);
	  pageContext.setAttribute("dtos",dtos);
	  if(cnt != 0){
		 // ArrayList<BoradDto> boardList = dao.getBoardList(startRow, pageSize);
	  }
%>
...
<body>
    <header>
        ...
        <div class="primary-menu">
   		 ...
   		 <h1>MBTI별 게시판</h1>
   		<div class="brt">
    		<button type="button" class="btn btn-primary" onclick="location.href='BInsert.do'" style="border-radius: 12px;">글쓰기</button>
    	</div>
    	...
    	<div class = "tborad">
		   <table class="table table-dark table-hover" style="width:80%;table-layout:auto;margin:auto"  >
			  ...
			  %>
                    <tr>
                        <th><%=j++ %></th>
                        <th><%=dto.getBoradtype() %></th>
                        <th><a href="BOneList.do?bid=<%=dto.getBid() %>&pageNum=<%=pageNum%>&boradtype=<%=dto.getBoradtype()%>"><%=dto.getBtitle() %></a>
                            <%if(dao.getCmtCount(Integer.parseInt(dto.getBid())) !=0){ %>
                            <a style="color: red;">[<%=dao.getCmtCount(Integer.parseInt(dto.getBid()))%>]</a>
                            <%} %>
                        </th>
                        <th><%=dto.getBname() %></th>
                        <th><%=dto.getBdate() %></th>
                        <th><%=dto.getBoradview() %></th>
