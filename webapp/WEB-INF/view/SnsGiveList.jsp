...
String id2 = (String)session.getAttribute("id");
	    ...
	    int j = 0;
	    //게시글 갯수
	    int cnt = dao.getSnsGiveCount(id2);
	    //출력할 게시글 갯수
	  	int pageSize = 5; 
	 	//선택한페이지번호
	  ...
	  ArrayList<SnsDto> dtos;
		//게시글 출력
	        ...
	        dtos = dao.giveSnsList(id2,startRow, pageSize);
    ...
    <div class = "container">
		...
		if(cnt != 0){
			...
			if(startPage > pageBlock){
				%>
                        <a href="SnsGiveList.do?pageNum=<%=startPage-pageBlock%>"> 이전 </a>
                        <%
			}
			...
			for(int k=startPage; k<=endPage; k++){
				%>
                        <a href="SnsGiveList.do?pageNum=<%=k%>"> <%=k%> </a>
                        <%
			}
			//다음
			...
			if(endPage < pageCount){
				%>
                        <a href="SnsGiveList.do?pageNum=<%=startPage+pageBlock%>"> 다음 </a>
                        <%
			}
			%>
                    </div>
